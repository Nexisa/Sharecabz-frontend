    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (selectedAsset.uri) {
          ...
          dispatch(updateProfile({ field: 'pic', value: selectedAsset.uri })); // Dispatch action here
          setIsChanged(true);
          Toast.show({
            type: 'success',
            text1: 'Success',
      ...
      const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
      console.error('Error selecting image:', errorMessage);
      Toast.show({
        type: 'error',
        text1: 'Error',
  ...
  const handleConfirmChanges = async () => {
    ...
    try {
      ...
      if (profileImage && profileImage.uri) {
        ...
        formData.append('image', blob, 'profileImage.jpg'); // Use the blob here
      }
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: `Failed to update profile: ${errorMessage}`,
      });
    } finally {
      setIsLoading(false);
    }
